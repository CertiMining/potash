 ...
 */
import { type Digest, bytesEqual } from "./bytes.ts";
import { RegistryFailure } from "./errors.ts";
import { canonicalizeBytes, isCanonicalTenure } from "./canonical.ts";
import { ed25519Verify, keccak256 } from "./hash.ts";
import { assetPreimage, genesisHeadPreimage, type LeafFields, leafPreimage, stepHeadPreimage } from "./preimage.ts";
...
export function assetCommitment(j: Uint8Array, r: Uint8Array, tenure: Uint8Array): Digest {
...
export function assetCommitmentFromRaw(j: Uint8Array, r: Uint8Array, rawTenure: Uint8Array): Digest {
  return assetCommitment(j, r, canonicalizeBytes(rawTenure));
}

/** §1.3 `h₀`. A schema this engine does not implement is 0x0F (V-N-13). */
export function genesisHead(assetCommitmentDigest: Digest, schemaVersion: number): Digest {
...
export function leafFieldsOf(r: RecordLeafInput): LeafFields {
  return {
    seq: r.seq,
...
export function leafPreimageOf(assetCommitmentDigest: Digest, r: RecordLeafInput): Uint8Array {
  return leafPreimage(assetCommitmentDigest, leafFieldsOf(r));
}

export function leafDigest(assetCommitmentDigest: Digest, r: RecordLeafInput): Digest {
  return keccak256(leafPreimageOf(assetCommitmentDigest, r));
}

/** §1.3 `hₙ₊₁ = Keccak256(TAG_HEAD ‖ hₙ ‖ leafₙ₊₁)`. */
export function advanceHead(prevHead: Digest, leaf: Digest): Digest {
 ...
 */
export function isWellFormedPayloadUri(uri: Uint8Array): boolean {
  if (uri.length < 1 || uri.length > 128) return false;
  for (const b of uri) {
    // Printable ASCII, with space and every control byte excluded.
    if (b <= 0x20 || b >= 0x7f) return false;
  }
  let text = "";
  for (const b of uri) text += String.fromCharCode(b);
  for (const scheme of URI_SCHEMES) {
    if (text.startsWith(scheme) && text.length > scheme.length) return true;
  }
  return false;
}

/** INV-STATE-06 / 06a: what the engine noticed, never a verdict, and never a rejection path. */
export function computeFlags(state: ChainState, category: number): number {
  let flags = 0;
 ...
 */
export function applyRecord(state: ChainState, r: RecordLeafInput): Applied {
...
export function commit(state: ChainState, r: RecordLeafInput, applied: Applied): ChainState {
  return {
