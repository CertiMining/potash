 ...
 */
import { concat, i64le, u16le, u64le, bytesEqual } from "./bytes.js";
import { RegistryFailure } from "./errors.js";
import { TAG_ASSET, TAG_HEAD, TAG_LEAF, TAG_LEN, TAG_MTL0, TAG_MTN1, TAG_PAD, TAG_PRF, TAG_SPI } from "./tags.js";
...
function fixed(name, b, len) {
    if (b.length !== len)
        throw new RegistryFailure("MalformedPayload", `${name} must be ${len} bytes, got ${b.length}`);
    return b;
}
/**
 ...
 */
export function readTagged(preimage, expectedTag) {
    if (preimage.length < TAG_LEN)
        throw new RegistryFailure("MalformedPayload", "preimage shorter than its tag");
...
export function assetPreimage(j, r, tenure) {
...
export function genesisHeadPreimage(c, schemaVersion) {
...
export function leafPreimage(c, f) {
