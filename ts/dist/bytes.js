...
export function fromHex(s, expectedLen) {
    if (typeof s !== "string" || !HEX_RE.test(s))
        throw new TypeError(`not hex: ${String(s)}`);
    const body = s.startsWith("0x") || s.startsWith("0X") ? s.slice(2) : s;
    if (body.length % 2 !== 0)
        throw new TypeError(`odd-length hex: ${s}`);
...
export function i64le(v) {
    if (v < -(2n ** 63n) || v > 2n ** 63n - 1n)
        throw new RangeError(`i64 out of range: ${v}`);
    const b = new Uint8Array(8);
    new DataView(b.buffer).setBigInt64(0, v, true);
    return b;
}
export function readU16le(b, off) {
...
export function readI64le(b, off) {
...
export function utf8StrictDecode(b) {
...
export function base64Decode(s) {
    if (typeof s !== "string")
        throw new TypeError("base64: not a string");
    if (s.length % 4 !== 0)
        throw new TypeError("base64: length is not a multiple of four");
    let pad = 0;
    if (s.endsWith("=="))
        pad = 2;
    else if (s.endsWith("="))
    ...
    if (body.includes("="))
        throw new TypeError("base64: padding inside the body");
    ...
    for (const ch of body) {
        ...
        if (v < 0)
            throw new TypeError(`base64: invalid character ${JSON.stringify(ch)}`);
        acc = (acc << 6) | v;
        bits += 6;
        if (bits >= 8) {
            bits -= 8;
    ...
    if (bits > 0 && (acc & ((1 << bits) - 1)) !== 0)
        throw new TypeError("base64: non-canonical padding bits");
    if (o !== out.length)
        throw new TypeError("base64: short decode");
    return out;
}
export function base64Encode(b) {
    let out = "";
