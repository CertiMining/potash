 ...
 */
import { concat, u64le, utf8 } from "../bytes.js";
import { isOnCurve, sha256 } from "../hash.js";
...
export function createProgramAddress(seeds, programId) {
    if (seeds.length > MAX_SEEDS)
        throw new RangeError("too many seeds");
    for (const s of seeds)
        if (s.length > MAX_SEED_LEN)
            throw new RangeError("seed is longer than 32 bytes");
    if (programId.length !== 32)
        throw new RangeError("program id must be 32 bytes");
...
export function deriveLogConfigAddress(programId) {
    return findProgramAddress([SEED_LOG_CONFIG], programId);
}
/** `CheckpointAccount` PDA, seeds ["cm_ckpt", epoch_le]. */
export function deriveCheckpointAddress(programId, epoch) {
