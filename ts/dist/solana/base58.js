...
export function base58Decode(s) {
    if (typeof s !== "string" || s.length === 0)
        throw new TypeError("base58: empty");
    const magnitude = []; // little-endian base-256 digits
    for (const ch of s) {
        const value = ALPHABET.indexOf(ch);
        if (value < 0)
            throw new TypeError(`base58: invalid character ${JSON.stringify(ch)}`);
        let carry = value;
    ...
    while (magnitude.length > 0 && magnitude[magnitude.length - 1] === 0)
        magnitude.pop();
    let leadingZeros = 0;
    for (const ch of s) {
        if (ch !== ALPHABET[0])
    ...
    for (let i = 0; i < magnitude.length; i++)
        out[leadingZeros + i] = magnitude[magnitude.length - 1 - i];
    return out;
}
