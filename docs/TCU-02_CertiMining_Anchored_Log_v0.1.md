...
**The discriminator, and how to compute it (added in v0.1.18 on this branch).** Both accounts open with Anchor's 8-byte account discriminator: the first eight bytes of `SHA-256("account:" ‖ N)`, where `N` is the account's struct name as this table gives it, taken as ASCII with no separator beyond the colon. For this program that is `LogConfig` → `1c f0 75 7f 1a a6 bf 37` and `CheckpointAccount` → `4d 11 99 cb 01 ec 47 59`. Through v0.1.17 this table named the field and its offset without saying how its bytes are derived, so an implementer working from this document alone could not produce or check them, and the only remaining option was to accept whatever an account carried. A reader is not required to trust the two constants: they are `SHA-256` of a published string, and the deployed accounts carry them.
...
**INV-DISC-01.** The package never contains the epoch key, sibling preimages, or any other asset's data, and carries no tenure, jurisdiction or registry value in clear. `c` appears inside `preimage_borsh` and nowhere else, because the QP signature is over those bytes (INV-ENC-03) and a verifier cannot check it without them. **The consequence, stated plainly: anyone holding one package can recompute `c` and recognise every other package the issuer releases for the same asset.** `slot_index` is pseudorandom and carries no information (INV-TREE-03).

*Corrected in v0.1.18 on this branch.* Through v0.1.17 this invariant read "the package never contains `c`", which §2.5 and INV-ENC-03 make unachievable: `preimage_borsh` is the 161-byte leaf preimage and §1.3 puts `c` at its bytes 8 to 39. INV-STATE-03 already said the opposite — that `c` may leave the issuer's control inside a package the issuer chose to release — so the document contradicted itself, and the version that could not be built was the one removed.
...
| V-Z-05 | **No asset identifier escapes.** Grep the full on-chain byte history for `c`, tenure IDs, jurisdiction codes and registry codes. Grep every disclosure package for tenure IDs, jurisdiction codes and registry codes, and for `c` outside `preimage_borsh`. | Zero occurrences on-chain. In the disclosure package, only the fields §2.5 lists; no tenure, jurisdiction or registry value in clear; and no occurrence of `c` outside `preimage_borsh`, where INV-DISC-01 places it and the QP signature requires it. *Amended with INV-DISC-01 (v0.1.18 on this branch): a grep for `c` across the whole package tested a condition §2.5 cannot satisfy.* |
