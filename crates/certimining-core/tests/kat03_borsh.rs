...
use certimining_core::{
    ...
    Preimage, PreimageBuf, PrfPreimage, RealLeafPreimage, SpiPreimage, StepHeadPreimage,
};
...
#[test]
fn kat03_the_prf_preimage_is_borsh_of_its_fields() {
    // `x` under the slot-assignment use: the code, then the submission identifier (§1.1, D-59).
    let mut x = vec![0x02u8];
    x.extend_from_slice(&SUBMISSION);
    let want = borsh_fields(&[
        to_vec(b"CMv1PRF0").unwrap(),
        to_vec(&PRF_KEY).unwrap(),
        // INV-ENC-04's `u16` length, which is where this differs from Borsh's own `u32` framing for
        // a byte string.
        to_vec(&17u16).unwrap(),
        x.clone(),
    ]);
    ...
    assert_eq!(bytes_of(&p), want);
    assert_eq!(bytes_of(&p).len(), 59, "8 + 32 + 2 + 17 (D-59)");
}
